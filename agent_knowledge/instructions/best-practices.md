...
- LARGE FILES (>300 lines) with write_file: a single write_file call may be truncated by max_tokens. Split into chunks using the append flag — first call without append (overwrites), then write_file again with the same path and `append: true` for each subsequent chunk. NEVER fall back to shell heredocs (`cat > file << EOF`) — shell may run against a different filesystem than the build directory and your changes will be lost on deploy.