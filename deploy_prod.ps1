# ============================================================
#  PRODUCTION deploy — apps-father.com
#  Server: 204.168.219.20
#  App dir: /opt/apps-father
#  PM2:     apps-father
#  DB:      apps_father_db  /  apps_father
# ============================================================
...
Write-Host "=== [PROD] Building ===" -ForegroundColor Cyan
npx tsc --project tsconfig.json
if ($LASTEXITCODE -ne 0) { Write-Host "Build failed!" -ForegroundColor Red; exit 1 }
Write-Host "Build OK" -ForegroundColor Green

Write-Host "=== [PROD] Uploading dist ===" -ForegroundColor Cyan
scp -r dist/* "${SERVER}:${APP_DIR}/dist/"
if ($LASTEXITCODE -ne 0) { Write-Host "Upload failed!" -ForegroundColor Red; exit 1 }
Write-Host "Upload OK" -ForegroundColor Green

Write-Host "=== [PROD] Uploading mini_app ===" -ForegroundColor Cyan
ssh $SERVER "mkdir -p ${APP_DIR}/mini_app"
scp -r mini_app/* "${SERVER}:${APP_DIR}/mini_app/"
Write-Host "Mini App OK" -ForegroundColor Green

Write-Host "=== [PROD] Uploading landing ===" -ForegroundColor Cyan
...
Write-Host "=== [PROD] Syncing package.json & deps ===" -ForegroundColor Cyan
...
Write-Host "=== [PROD] Syncing Prisma schema ===" -ForegroundColor Cyan
...
Write-Host "=== [PROD] Running DB migrations ===" -ForegroundColor Cyan
...
Write-Host "=== [PROD] Syncing skills ===" -ForegroundColor Cyan
scp -r skills/* "${SERVER}:${APP_DIR}/skills/"
Write-Host "Skills OK" -ForegroundColor Green

Write-Host "=== [PROD] Restarting ===" -ForegroundColor Cyan
ssh $SERVER "cd ${APP_DIR} && pm2 restart ${PM2} --update-env --kill-timeout 300000"
if ($LASTEXITCODE -ne 0) { Write-Host "Restart failed!" -ForegroundColor Red; exit 1 }

Write-Host "=== [PROD] Deployed to PRODUCTION! ===" -ForegroundColor Green
